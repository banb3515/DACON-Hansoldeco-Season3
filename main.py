...
- Summarize your responses concisely with only key points.
- Do not include introductions, background information, or additional explanations.
- Avoid phrases like "We suggest the following measures:".
- Clearly list specific safety measures and action plans.
- Base your answers solely on the provided documents (reference materials).
</rules>
<documents>
${documents}
</documents>\
...
- 구체적인 안전 대책과 조치 계획을 명확하게 나열하세요.
- 제공된 문서(참고 자료)를 기반으로 답변하세요.
</rules>
<documents>
${documents}
</documents>\
""")
    ...
    def _inference(question: str):
        ...
        reasoning_prompt = REASONING_SYSTEM_PROMPT.substitute(documents="\n".join(documents))
        chat_prompt = CHAT_SYSTEM_PROMPT.substitute(documents="\n".join(documents))
...
def create_result_embeddings(results: list[dict[str, str]], model_name: str = "jhgan/ko-sbert-sts") -> np.ndarray:
    ...
    batch_size = min(64, len(results) // 10 + 1) 
...
def save_results(results: list[dict[str, str]], embeddings: np.ndarray, data_dir: str = "./data", submissions_dir: str = "./submissions") -> None:
    """Save results
